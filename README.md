...
<p align="center">
    <a href="https://youtu.be/1pJjXedOQc0">
        <img src="https://img.youtube.com/vi/1pJjXedOQc0/maxresdefault.jpg" alt="Xem video demo">
    </a>
</p>
...
| File                             | Mô tả                                      |
| -------------------------------- | ------------------------------------------ |
| `extensions/Bybit-Wallet-*.crx` | Tiện ích mở rộng Bitget Wallet.            |
| `w_bybit.py`                     | Xử lý các thao tác liên quan đến Biget.    |
| `index.py`                       | File khởi chạy chương trình chính.         |
| `config_example.txt`             | File cấu hình mẫu cho tool.                |
| `intro.png`                      | Hình ảnh giới thiệu.                       |
| `picture.png`                    | Hình ảnh được chèn vào post làm nhiệm vụ.                       |
...
2. **Tạo môi trường ảo**
   ...
   ```sh
   python -m venv venv-browserkit1.1.2
   ```
...
3. **Kích hoạt môi trường ảo**
   ...
   * **Windows**
     ...
     ```sh
     venv-browserkit1.1.2\Scripts\activate
     ```
   * **Linux/macOS**

     ```sh
     source venv-browserkit1.1.2/bin/activate
     ```
...
- Mỗi dòng chứa thông tin một profile theo cấu trúc:
  ```plaintext
  <tên_profile>|<mật_khẩu_ví_bybit>|<12_seeds (tuỳ chọn 2)>|<proxy (tuỳ chọn)>
  ```

  Trong đó:

    - `tên_profile`: Tên của profile.
    - `mật_khẩu_ví_bybit`: Mật khẩu đăng nhập ví Bybit (gồm chữ Hoa).
...
- Ví dụ:
  ...
  profile3|12345678|word1 ... word12                      // không proxy
  profile4|12345678                                       // không mật khẩu, không seeds, không proxy
  ```
